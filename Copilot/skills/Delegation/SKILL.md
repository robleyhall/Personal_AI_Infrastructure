...
**This skill is ported in degraded form.** Copilot CLI's agent system is a
strict subset of Claude Code's, so much of the routing below does not apply
verbatim. Map everything through these rules:

| Original Capability | Copilot Reality |
|---|---|
| `Task(subagent_type="Engineer"/"Architect"/"Algorithm"/...)` (specialized types) | **Collapse to one agent.** Use `task` tool with `agent_type: "general-purpose"` and put the specialization into the prompt. |
| `Task(..., model="haiku"/"sonnet"/"opus")` | Ignored. Agent runs on Copilot's configured model. |
| `TeamCreate` / `TaskCreate` / `SendMessage` (persistent agent teams) | **Not available.** Use sequential or parallel one-shot `task` calls instead; there is no shared task list or inter-agent messaging. |
| `run_in_background: true` | Use `mode: "background"` on the `task` tool. |
| Voice/prosody per agent | Voice server is macOS `say`; prosody metadata is harmless dead weight. |

**When a user requests an "agent team" or "swarm":** explain that Copilot has
no TeamCreate; fall back to parallel `task` calls in background mode and
coordinate through shared files (`plan.md`, `tasks/todo.md`, or the SQL
`todos` table) rather than `SendMessage`.

**When the prose below mentions specialized agent types:** treat them as
prompt flavoring. The actual dispatcher call is always:

```
task(agent_type: "general-purpose", prompt: "<specialized instructions>", mode: "background"?)
```
...
|-------------|--------|------|-------------|
| "**custom agents**", "spin up agents", "launch agents" | **Agents Skill** (ComposeAgent) | `task(agent_type="general-purpose", prompt=<ComposeAgent output>)` | Unique personalities, voices, colors via trait composition |
...
Use `task(agent_type="general-purpose")` with these specialized agents:
...
```
task(agent_type="general-purpose", isolation: "worktree", prompt="...")
```
...
- Can combine with `mode: "background"` for non-blocking isolated work
- **Built-in agents with `isolation: worktree` in frontmatter** (Engineer, Architect) auto-isolate on every spawn

### 3. Background Agents

Run agents with `mode: "background"` for non-blocking parallel work:
...
**Action:** Invoke the **Agents skill** → run `ComposeAgent.ts` → launch with `task(agent_type="general-purpose")`
...
task(agent_type="general-purpose", prompt=<ComposeAgent JSON .prompt field>)
```
...
3. task(agent_type="general-purpose", team_name="my-project", name="auth-engineer")  # Spawn teammate
...
```
task(agent_type="general-purpose", model="haiku", max_turns=3, prompt="...")
```
...
```
task(agent_type="general-purpose", prompt="...")  # or specialized agent type
```
