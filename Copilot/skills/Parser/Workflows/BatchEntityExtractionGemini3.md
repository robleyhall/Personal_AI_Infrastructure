...
URLS=("$@")
SKILL_DIR="$HOME/.pai/skills/parser"
...
const entityIndexPath = `${process.env.HOME}/.pai/skills/parser/entity-index.json`;
...
processedArticles.forEach((article: any, index: number) => {
  const timestamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, -5);
  const filename = `${process.env.HOME}/.pai/skills/parser/output/${timestamp}_batch-${index + 1}.json`;
