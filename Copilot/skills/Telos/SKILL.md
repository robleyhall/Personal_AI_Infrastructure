...
1. **Personal TELOS** - {PRINCIPAL.NAME}'s life context system (beliefs, goals, lessons, wisdom) at `~/.pai/USER/TELOS/`
...
|--------------|---------|----------|
| "my TELOS", "my goals", "my beliefs", "add to TELOS" | Personal TELOS | `~/.pai/USER/TELOS/` |
...
All files located in `~/.pai/USER/TELOS/`:
...
```bash
# View specific file
read ~/.pai/USER/TELOS/GOALS.md
read ~/.pai/USER/TELOS/BELIEFS.md

# View recent updates
read ~/.pai/USER/TELOS/updates.md
```
...
1. **Dual Context** - Handles both personal and project TELOS seamlessly
   - Personal TELOS: `~/.pai/USER/TELOS/` (in CORE USER directory)
...
**Remember:** Personal TELOS files live at `~/.pai/USER/TELOS/` (in the CORE USER directory)
