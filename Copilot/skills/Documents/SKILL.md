...
`~/.pai/USER/SKILLCUSTOMIZATIONS/Documents/`
...
## Copilot Spike Status

> **Only the PDF sub-skill is installed in the Copilot CLI port.** DOCX, XLSX,
> and PPTX sub-skills are **not ported yet** — their upstream sources live at
> `Releases/v4.0.3/.claude/skills/Utilities/Documents/{Docx,Pptx,Xlsx}/` and
> reference Python/Node tooling that needs validation before a clean port.
> If a user asks for DOCX/XLSX/PPTX work, explain this limitation and offer
> to:
> 1. Port the relevant sub-skill now (mechanical substitution, but validate
>    script deps first), or
> 2. Fall back to a minimal solution using `bash` + `python` with
>    `python-docx` / `openpyxl` / `python-pptx` as a one-off.
...
| Word document, DOCX, create docx, edit docx, tracked changes, redlining | **Not ported** — see Copilot Spike Status above |
| Presentation, PPTX, slides, PowerPoint, speaker notes | **Not ported** — see Copilot Spike Status above |
| Spreadsheet, XLSX, Excel, formulas, financial model, data analysis | **Not ported** — see Copilot Spike Status above |
